...
public interface ReviewHeadRepository extends JpaRepository<ReviewHead, ReviewHeadId> {
    ...
    @Query("SELECT new hu.okrim.productreviewappcomplete.dto.DashboardReviewByMonthDTO(TO_CHAR(r.date, 'Month'), COUNT(r)) " +
            "FROM ReviewHead r " +
            "WHERE EXTRACT(YEAR FROM r.date) = :currentYear " +
            "GROUP BY TO_CHAR(r.date, 'Month'), EXTRACT(MONTH FROM r.date) " +
    ...
    @Query(
            "WITH domestic_review_count AS (" +
                    ...
                    ") " +
                    "SELECT ROUND(domestic * 1.0 / NULLIF(total, 0) * 100, 2)" +
    ...
    @Query(value = "WITH reviews_per_brand AS ( " +
            ...
            "ROUND((AVG(rh.value_for_price) + COALESCE(AVG(rb.score), 5)) / 2.0, 2) AS avg_total " +
            "FROM review_head rh " +
            ...
            ") " +
            "SELECT range, " +
            "ROUND(COUNT(1) * 100.0 / NULLIF((SELECT COUNT(1) FROM product_review_avg), 0), 2) AS percentage " +
